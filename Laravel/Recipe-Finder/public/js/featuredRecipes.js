...
function renderFeaturedCards(grid, recipes) {
    if (!recipes || !recipes.length) {
        grid.innerHTML =
            '<p style="padding:20px;color:var(--ink-soft)">No featured recipes found.</p>';
        return;
    }
...
function featuredCardHTML(r) {
    ...
    const tags = Array.isArray(r.tags)
        ? r.tags
              .slice(0, 2)
              .map(
                  (t) => `<span class="featured-card-tag">${escHtml(t)}</span>`,
              )
              .join("")
        : "";
  ...
  <article class="featured-card">
    ...
    <div class="featured-card-body">
      ...
      <div class="recipe-card-actions" style="margin-top: 1rem;">
        <button 
          type="button" 
          class="btn btn-outline btn-sm featured-view-btn"
...
function openFeaturedModal(safeJson) {
    ...
    const ingredients = ingredientSource
        .map((t) => `<span class="modal-ingredient-tag">${escHtml(t)}</span>`)
        .join("");
